...
describe('writeExportPackage', () => {
  it('streams approved entries and produces checksums without full-package buffering', async () => {
    ...
    const result = await writeExportPackage([
      { path: 'identity/profile.json', mediaType: 'application/json', stream: bytes('{"id":"opaque"}') },
    ], output, { maxBytes: 1024 * 1024, maxEntries: 5 });
  ...
  it.each(['../secret.json', '/absolute.json', 'a\\b.json', 'script.exe', 'a//b.json'])('rejects unsafe path %s', async (path) => {
    await expect(writeExportPackage([
      { path, mediaType: 'application/json', stream: bytes('{}') },
    ], new PassThrough(), { maxBytes: 1000, maxEntries: 5 })).rejects.toThrow('EXPORT_ENTRY_INVALID');
  });
  ...
  it('rejects duplicate paths and bounded count/bytes', async () => {
    ...
    await expect(writeExportPackage([entry, entry], new PassThrough(), { maxBytes: 1000, maxEntries: 5 })).rejects.toThrow('EXPORT_ENTRY_DUPLICATE');
    await expect(writeExportPackage([entry], new PassThrough(), { maxBytes: 1, maxEntries: 5 })).rejects.toThrow('EXPORT_SIZE_EXCEEDED');
    await expect(writeExportPackage([entry], new PassThrough(), { maxBytes: 1000, maxEntries: 0 })).rejects.toThrow('EXPORT_LIMIT_INVALID');
  });
  ...
  it('destroys the destination when an input stream fails', async () => {
    ...
    ], output, { maxBytes: 1000, maxEntries: 5 })).rejects.toThrow();
