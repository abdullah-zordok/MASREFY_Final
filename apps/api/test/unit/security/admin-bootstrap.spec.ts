...
describe('Admin bootstrap arguments', () => {
  it('requires separate subject and approver plus a bounded reason', () => {
    expect(parseBootstrapArgs(['--user-id','user_1','--approved-by','reviewer_2','--reason','Approved deployment bootstrap'])).toEqual({
      userId:'user_1',approvedBy:'reviewer_2',reason:'Approved deployment bootstrap',
    });
  });
