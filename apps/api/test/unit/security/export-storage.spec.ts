...
describe('ExportStorage', () => {
  ...
  it('uploads privately, validates metadata, signs briefly, and deletes', async () => {
    fetcher
      ...
      .mockResolvedValueOnce(
        new Response(null, { status: 200, headers: { 'content-length': '3', etag: '"etag"' } }),
      )
      .mockResolvedValueOnce(
        new Response(
          JSON.stringify({
            signedURL: '/storage/v1/object/sign/report-exports/exports/id/file.zip?token=opaque',
          }),
          { status: 200 },
        ),
      )
    ...
    await expect(storage.head('exports/id/file.zip', 3)).resolves.toEqual({
      bytes: 3,
      etag: 'etag',
    });
    await expect(storage.sign('exports/id/file.zip', 120)).resolves.toContain(
      '/storage/v1/object/sign/',
    );
    await expect(storage.delete('exports/id/file.zip')).resolves.toBeUndefined();
    expect(
      fetcher.mock.calls.every(
        ([, init]) => new Headers(init?.headers).get('Authorization') === 'Bearer service-secret',
      ),
    ).toBe(true);
  });

  it.each(['../file.zip', '/file.zip', 'exports/id/file.exe', 'exports//file.zip'])(
  ...
  it('maps malformed and failed responses without exposing body, key, or credential', async () => {
    fetcher.mockResolvedValueOnce(
      new Response('service-secret exports/id/file.zip', { status: 500 }),
    );
    const storage = new ExportStorage(config as never, fetcher);
  ...
  it('rejects unsafe signed URLs and metadata mismatch', async () => {
    fetcher
      ...
      .mockResolvedValueOnce(
        new Response(
          JSON.stringify({
            signedURL: '/storage/v1/object/sign/report-exports/exports/other/file.zip?token=opaque',
          }),
          { status: 200 },
        ),
      );
    const storage = new ExportStorage(config as never, fetcher);
    await expect(storage.head('exports/id/file.zip', 3)).rejects.toThrow(
      'EXPORT_STORAGE_INTEGRITY_FAILED',
    );
    await expect(storage.sign('exports/id/file.zip', 120)).rejects.toThrow(
