...
describe('ReportsService summaries', () => {
  ...
  it('validates input, resolves the owner timezone, and caches by current ledger version', async () => {
    ...
    const first = await service.getReportSummary(principal, { type: 'financial_summary', period: 'monthly', currency: 'SAR' }, 'r1', new Date('2026-09-04T00:00:00Z'));
    const second = await service.getReportSummary(principal, { type: 'financial_summary', period: 'monthly', currency: 'SAR' }, 'r2', new Date('2026-09-04T00:00:00Z'));
    ...
    expect(calls[0]?.[2]).toMatchObject({ startDate: '2026-09-01', endDate: '2026-09-30', timezone: 'Asia/Riyadh' });
  });
  ...
  it('keeps dashboard and report namespaces separate and maps validation errors safely', async () => {
    const repository = {
      ...
      getSummary: jest.fn().mockResolvedValue(response),
      getHome: jest.fn().mockResolvedValue({ ...response, balances: [], planning: {}, recentItems: [] }),
    };
    const service = new ReportsService(repository as never);
    await service.getReportSummary(principal, { type: 'financial_summary', period: 'annual' }, 'a', new Date('2026-09-04T00:00:00Z'));
    await service.getDashboardHome(principal, { period: 'annual' }, 'b', new Date('2026-09-04T00:00:00Z'));
    expect(repository.getSummary).toHaveBeenCalledTimes(1);
    expect(repository.getHome).toHaveBeenCalledTimes(1);
    await expect(service.getReportSummary(principal, { type: 'financial_summary', period: 'monthly', currency: 'sar' }, 'bad')).rejects.toMatchObject({ status: 400 });
  });
});
