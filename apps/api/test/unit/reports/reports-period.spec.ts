...
describe('Phase 10 report periods', () => {
  it('selects the next deterministic 08:00 local boundary', () => {
    expect(
      firstScheduleRun('monthly', 'Asia/Riyadh', new Date('2026-09-04T00:00:00Z')).toISOString(),
    ).toBe('2026-10-01T05:00:00.000Z');
    expect(firstScheduleRun('annual', 'UTC', new Date('2026-09-04T00:00:00Z')).toISOString()).toBe(
  ...
  it('chooses the first valid minute after a DST gap and the earlier fold instant', () => {
    expect(zonedDateTimeToInstant('2026-03-08', '02:30', 'America/New_York').toISOString()).toBe(
      '2026-03-08T07:00:00.000Z',
    );
    expect(zonedDateTimeToInstant('2026-11-01', '01:30', 'America/New_York').toISOString()).toBe(
