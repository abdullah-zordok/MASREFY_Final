...
describeLiveDatabase('signed Clerk webhook flow', () => {
  ...
  const subjects = [
    subject,
    'webhook_e2e_conflict',
    'webhook_e2e_ordered',
    'webhook_e2e_retry',
    'webhook_e2e_reconciled',
  ];
  const eventIds = [
    eventId,
    'msg_webhook_e2e_conflict',
    'msg_webhook_e2e_order_a',
    'msg_webhook_e2e_order_b',
    'msg_webhook_e2e_retry',
    'msg_webhook_e2e_retention',
  ];

  function headers(body: string, deliveryId = eventId) {
    const timestamp = Math.floor(Date.now() / 1000);
    const signature = createHmac('sha256', secretBytes)
      .update(`${deliveryId}.${String(timestamp)}.${body}`)
      .digest('base64');
    return {
      'svix-id': deliveryId,
      ...
      providers: [
        { provide: IdentityRepository, useValue: repository },
        {
          ...
          useValue: {
            ...
            getRequired: jest.fn().mockReturnValue(secret),
          },
        },
      ],
    }).compile();
  ...
  it('durably accepts a signed duplicate and produces one converged effect', async () => {
    ...
    const evidence = await asMigration((client) =>
      client.query<{ inbox: string; profiles: string; events: string }>(
        `select
  ...
  it('rejects a signed delivery-ID conflict without duplicating durable evidence', async () => {
    ...
    await request(server)
      .post('/webhooks/clerk')
      .set(headers(created, deliveryId))
      .set('content-type', 'application/json')
      .send(created)
      .expect(202);
    await request(server)
      .post('/webhooks/clerk')
      .set(headers(deleted, deliveryId))
      .set('content-type', 'application/json')
      .send(deleted)
      .expect(409);
    await repository.processNextClerkWebhook(() => Promise.resolve(null), 3);
    const inbox = await asMigration((client) =>
  ...
  it('converges out-of-order signed signals on current Clerk state and retries a crash', async () => {
    ...
    await request(server)
      .post('/webhooks/clerk')
      .set(headers(retryBody, 'msg_webhook_e2e_retry'))
      .set('content-type', 'application/json')
      .send(retryBody)
      .expect(202);
    await expect(
      repository.processNextClerkWebhook(() => Promise.reject(new Error('simulated crash')), 3),
    ...
    const profiles = await asMigration((client) =>
      client.query<{
        id: string;
        status: string;
