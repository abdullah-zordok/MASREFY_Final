...
describeLiveDatabase('device owner HTTP flow', () => {
  ...
  const owner = {
    userId: 'device_e2e_owner',
    sessionId: 'device_e2e_session',
    factorAgeSeconds: 30,
  };
  const other = {
    userId: 'device_e2e_other',
    sessionId: 'device_e2e_other_session',
    factorAgeSeconds: 30,
  };
  ...
  beforeAll(async () => {
    ...
    await asMigration((client) =>
      client.query(`insert into public.profiles(id,status) values($1,'active'),($2,'active')`, [
        owner.userId,
        other.userId,
      ]),
    );
    const repository = new IdentityRepository(
      pool,
      new PushTokenCrypto(Buffer.alloc(32, 41), [{ id: 'active', key: Buffer.alloc(32, 42) }]),
    );
    const module = await Test.createTestingModule({
      ...
      providers: [
        ...
        {
          provide: ClerkClientService,
          useValue: { revokeSession: jest.fn().mockResolvedValue('revoked') },
        },
  ...
  it('registers, lists, isolates, and revokes one owner device', async () => {
    ...
    const registration = await request(server)
      ...
      .send({
        deviceFingerprint: 'device-e2e-fingerprint',
        platform: 'android',
        appVersion: '1.0.0',
      })
      .expect(201);
    ...
    await request(server)
      ...
      .set('Idempotency-Key', 'device-e2e-other')
      .expect(404);
    active = owner;
    await request(server)
      .delete(`/api/v1/me/devices/${deviceId}`)
      .set('Idempotency-Key', 'device-e2e-revoke')
      .expect(204);
    await request(server)
      .delete(`/api/v1/me/devices/${deviceId}`)
      .set('Idempotency-Key', 'device-e2e-repeat')
      .expect(204);
  });
});
