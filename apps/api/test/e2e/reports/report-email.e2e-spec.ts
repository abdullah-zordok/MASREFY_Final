      ...
      getRequired: (key: string) =>
        (
          ({
            ...
            EMAIL_SMTP_PORT: 465,
            EMAIL_SMTP_USERNAME: 'fixture',
            EMAIL_SMTP_PASSWORD: 'fixture',
            EMAIL_FROM: 'reports@example.test',
            MASARIFI_EMAIL_SMTP_CONNECTION_TIMEOUT_MS: 250,
            MASARIFI_EMAIL_SMTP_SOCKET_TIMEOUT_MS: 500,
            SUPABASE_URL: 'https://project.supabase.co',
          }) as Record<string, unknown>
        )[key],
    };
    const smtp = new ReportsSmtp(smtpConfig as never, {
      sendMail: (message) => {
        sent.push(message);
    ...
    const repository = {
      ...
      listWork: jest
        ...
        .mockImplementation((kind: string) =>
          Promise.resolve(kind === 'report.email.deliver' && pending ? ['id'] : []),
        ),
      withWorkLock: jest
        .fn()
        .mockImplementation(
