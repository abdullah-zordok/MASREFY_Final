...
describe('Clerk webhook security boundary', () => {
  ...
  it('uses the official verifier without custom signature comparison or forwarded identity', () => {
    const source = readFileSync(
      resolve(__dirname, '../../../src/identity/clerk-webhook.controller.ts'),
      'utf8',
    );
  ...
  it.each([
    ...
    'rejects a %s signed delivery before persistence',
    async (_case, offset, overrideSignature) => {
  ...
  it('accepts an identical signed replay only through the durable duplicate path', async () => {
    ...
    const { controller, repository, response } = boundary();
    repository.receiveClerkWebhook
  ...
  it('keeps private payloads outside every client grant', () => {
    const migration = readFileSync(
      resolve(
        __dirname,
        '../../../../../supabase/migrations/20260827001200_clerk_webhook_grants.sql',
      ),
      'utf8',
    );
    expect(migration).toMatch(
      /grant select \(clerk_event_id, payload_hash\)[\s\S]+to masarifi_api/i,
    );
