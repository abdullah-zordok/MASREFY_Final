...
describe('device and push exposure boundaries', () => {
      ...
      listDevices: jest.fn().mockResolvedValue([
        {
          ...
          revokedAt: null,
          clerkSessionId: principal.sessionId,
          createdAt: new Date(),
          version: 1,
        },
      ]),
    };
    const response = await new IdentityService(repository as never).listDevices(principal, {
      limit: 50,
    });
  ...
  it('maps provider details to one safe error after retaining local revocation', async () => {
    ...
    const clerk = {
      revokeSession: jest
        .fn()
        .mockRejectedValue(new Error('CLERK_PROVIDER_UNAVAILABLE private detail')),
    };
    const promise = new IdentityService(repository as never, clerk as never).revokeDevice(
      principal,
      '4e971c69-210a-4c21-b535-5ad290d057df',
      'device-revoke-01',
    );
