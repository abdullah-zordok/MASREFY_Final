...
describe('Admin identity boundary', () => {
  ...
  const contract = load(
    readFileSync(
      resolve(root, 'apps/api/specs/002-auth-profiles-preferences-sessions/contracts/openapi.yaml'),
      'utf8',
    ),
  ) as { paths: Record<string, unknown> };

  it('defines no Admin identity/device/session/status/force-logout route', () => {
    expect(Object.keys(contract.paths)).not.toEqual(
  ...
  it('does not authorize from role headers or metadata', () => {
    expect(identitySource).not.toMatch(
      /x-admin-role|x-user-role|publicMetadata|privateMetadata|unsafeMetadata/i,
    );
  });
});
