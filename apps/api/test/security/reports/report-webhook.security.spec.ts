...
describe('report delivery webhook', () => {
  ...
  const captureDeliveryWebhook = jest.fn((id: string, hash: string) => {
    const previous = receipts.get(id);
    if (!previous) {
      receipts.set(id, hash);
      return 'new';
    }
    ...
    const module = await Test.createTestingModule({
      ...
      providers: [
        { provide: PlatformConfigService, useValue: { get: jest.fn().mockReturnValue(secret) } },
        {
          ...
          useValue: { captureDeliveryWebhook },
        },
      ],
    }).compile();
  ...
  function headers(body: string, id = 'evt-1', time = Math.floor(Date.now() / 1000)) {
    const signature = createHmac('sha256', secret)
      .update(`${id}.${String(time)}.${body}`)
      .digest('hex');
    return {
      'x-report-event-id': id,
      'x-report-timestamp': String(time),
      'x-report-signature': `v1=${signature}`,
    };
  }

  it('accepts an identical signed replay and rejects a conflicting replay', async () => {
    const body = JSON.stringify({ type: 'accepted', attemptId });
    await request(app.getHttpServer() as Parameters<typeof request>[0])
      .post('/webhooks/report-delivery')
      .set(headers(body))
    ...
    await request(app.getHttpServer() as Parameters<typeof request>[0])
      ...
      .send(body)
      .expect(202);
    const changed = JSON.stringify({ type: 'rejected', attemptId });
    await request(app.getHttpServer() as Parameters<typeof request>[0])
      .post('/webhooks/report-delivery')
      .set(headers(changed))
      .set('content-type', 'application/json')
      .send(changed)
      .expect(409);
    expect(captureDeliveryWebhook).toHaveBeenCalledWith(
      expect.stringMatching(/^sha256:[a-f0-9]{64}$/),
      expect.stringMatching(/^sha256:[a-f0-9]{64}$/),
    );
  });

  it('rejects a signed but unknown payload before recording a receipt', async () => {
    const body = JSON.stringify({ type: 'accepted', attemptId, recipient: 'hidden@example.test' });
    captureDeliveryWebhook.mockClear();
    await request(app.getHttpServer() as Parameters<typeof request>[0])
      .post('/webhooks/report-delivery')
      .set(headers(body, 'invalid-schema'))
      .set('content-type', 'application/json')
      .send(body)
      .expect(400);
    expect(captureDeliveryWebhook).not.toHaveBeenCalled();
  });
  ...
  it('rejects stale and forged signatures', async () => {
    ...
    await request(app.getHttpServer() as Parameters<typeof request>[0])
      .post('/webhooks/report-delivery')
      .set(headers(body, 'stale', Math.floor(Date.now() / 1000) - 301))
      .set('content-type', 'application/json')
      .send(body)
      .expect(401);
    await request(app.getHttpServer() as Parameters<typeof request>[0])
      .post('/webhooks/report-delivery')
      .set({ ...headers(body, 'forged'), 'x-report-signature': `v1=${'0'.repeat(64)}` })
      .set('content-type', 'application/json')
      .send(body)
      .expect(401);
  });
