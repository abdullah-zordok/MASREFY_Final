...
describe('ReportsStorage security', () => {
  const config = {
    getRequired: jest.fn((key: string) => key === 'SUPABASE_URL' ? 'http://storage.local' : key === 'NODE_ENV' ? 'test' : 'service-secret'),
  };
  ...
  it('uses an owner-hashed fixed key, exact content type, no upsert, and verifies bytes', async () => {
    ...
    const stored = await storage.upload(attemptId, 'owner-private', 'json', 'application/json', Readable.from('abc'), 100);
  ...
  it('accepts only short owner-scoped signed URLs and treats missing deletion as success', async () => {
    ...
    fetcher
      .mockResolvedValueOnce(new Response(JSON.stringify({ signedURL: `/storage/v1/object/sign/report-exports/${key}?token=opaque` }), { status: 200 }))
  ...
  it.each(['../file.pdf', 'reports/hash/file.pdf', 'reports/' + 'a'.repeat(64) + '/../../file.pdf'])('rejects traversal or malformed keys %s', async (key) => {
    const storage = new ReportsStorage(config as never, fetcher);
  ...
  it('rejects foreign origins and redacts storage responses and credentials', async () => {
    fetcher.mockResolvedValueOnce(new Response(JSON.stringify({ signedURL: 'https://evil.example/file' }), { status: 200 }));
    const storage = new ReportsStorage(config as never, fetcher);
    const key = storage.key(attemptId, 'owner-private', 'csv');
    const error = await storage.sign(key, 120).then(() => new Error('unexpected'), (value: unknown) => value as Error);
