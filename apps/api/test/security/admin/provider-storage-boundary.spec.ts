...
describe('provider and storage exceptional boundaries', () => {
  it('maps malformed private-storage responses to a stable non-secret error', async () => {
    const fetcher = jest.fn().mockResolvedValue({
      ok: true,
      json: () => Promise.resolve({ signedURL: 'http://unsafe.test/object' }),
    });
    const storage = new ExportStorage(
      {
        getRequired: jest.fn((key: string) =>
          key.includes('URL') ? 'https://storage.example.test' : 'secret-fixture',
        ),
        get: jest.fn(() => 100),
      } as never,
      fetcher as never,
    );
    await expect(storage.sign('exports/request/privacy.zip', 60)).rejects.toThrow(
