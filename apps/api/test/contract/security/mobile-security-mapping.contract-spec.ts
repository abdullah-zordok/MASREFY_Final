...
describe('Mobile security boundary mapping', () => {
  it('maps owner workflows and explicitly excludes raw evidence', () => {
    const mapping = readFileSync(resolve(__dirname, '../../../specs/003-admin-rbac-security/contracts/client-mapping.md'), 'utf8');
    ...
    expect(mapping).toMatch(/Raw\s+IP, token, provider payload, unrestricted user agent, or internal Storage reference\s+has no Mobile mapping/);
  });
});
