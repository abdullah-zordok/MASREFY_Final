...
describe('Phase 03 route authorization contract', () => {
  it('registers all 45 operations with the exact OpenAPI permission and status', () => {
    const contract = load(readFileSync(resolve(__dirname, '../../../specs/003-admin-rbac-security/contracts/openapi.yaml'), 'utf8')) as {
      paths: Record<string, Record<string, { operationId: string; 'x-permission'?: string; responses: Record<string, unknown> }>>;
    };
    const expected = Object.entries(contract.paths).flatMap(([path, methods]) =>
      Object.entries(methods).filter(([method]) => ['get','post','patch','delete'].includes(method)).map(([method, operation]) => ({
    ...
    expect(SECURITY_ROUTES.map(({ method, path, operation, status, permission }) => ({ method, path, operation, status, permission }))).toEqual(expected);
  ...
  it('never registers wildcard or client-derived permissions', () => {
    ...
    expect(SECURITY_ROUTES.every(({ permission }) => !permission || (known.has(permission) && !permission.includes('*')))).toBe(true);
  });
});
