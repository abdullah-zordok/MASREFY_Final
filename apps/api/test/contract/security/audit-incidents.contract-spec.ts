...
describe('audit and incident contract', () => {
  it('exposes only redacted evidence and an explicit incident timeline', () => {
    ...
    expect(contract.components.schemas.Incident?.required).toEqual(
      expect.arrayContaining(['timeline', 'version']),
    );
    expect(contract.components.schemas.AuditEvent?.required).toEqual(
      expect.arrayContaining(['safeMetadata', 'requestId']),
    );
    expect(
      JSON.stringify({
