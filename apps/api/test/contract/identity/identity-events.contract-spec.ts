...
describe('identity event contracts', () => {
  ...
  it('deduplicates and sorts the approved profile.updated fields', () => {
    expect(
      buildProfileUpdatedPayload('user_fixture_a', 4, ['timezone', 'locale', 'timezone']),
    ).toEqual({
      payloadVersion: 1,
      profileId: 'user_fixture_a',
      profileVersion: 4,
  ...
  it('builds bounded device payloads without session, fingerprint, or token evidence', () => {
    ...
    expect(JSON.stringify([registered, revoked])).not.toMatch(
      /sessionId|fingerprint|token|cipher|hash/i,
    );
  });
