...
describe('Phase 10 client/OpenAPI parity', () => {
  const contract = load(
    readFileSync('specs/010-reports-analytics-exports-email/contracts/openapi.yaml', 'utf8'),
  ) as ApiDocument;
  ...
  it('keeps every Mobile and Admin report operation in the frozen API contract', () => {
    ...
    for (const path of ['/api/v1/reports/summary', '/api/v1/reports', '/api/v1/report-schedules'])
      expect(mobile).toContain(path);
    for (const path of ['/api/v1/admin/exports', '/api/v1/admin/exports/'])
  ...
  it('keeps public enums exact and bounded', () => {
    ...
    expect(values('ReportType')).toEqual([
      'financial_summary',
      'category_spending',
      'budget_performance',
      'obligation_progress',
    ...
    expect(values('ReportStatus')).toEqual([
      'queued',
      'generating',
      'ready',
      'sending',
