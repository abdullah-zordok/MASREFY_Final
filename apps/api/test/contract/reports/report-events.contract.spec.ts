...
describe('report delivery events', () => {
  ...
  it('keeps delivery success and failure metadata exact and private', () => {
    expect(
      buildReportEvent('report.delivery_succeeded', {
        attemptId,
      ...
      schemaVersion: 1,
      data: { attemptId, acceptedByServerAt: '2026-09-04T00:00:00.000Z' },
    });
    expect(
      buildReportEvent('report.delivery_failed', {
        attemptId,
        errorCode: 'REPORT_SMTP_REJECTED',
        retryable: false,
        attemptCount: 1,
      }),
    ).not.toHaveProperty('data.recipient');
    expect(() =>
      buildReportEvent('report.delivery_failed', {
        attemptId,
        errorCode: 'REPORT_SMTP_REJECTED',
