...
describe('report generation HTTP contract', () => {
  ...
  it('registers request, list, status, and explicit delivery retry routes', () => {
    expect(REPORT_ROUTES).toEqual(
      expect.arrayContaining([
        ...
        expect.objectContaining({
          method: 'GET',
          path: 'api/v1/reports/:attemptId',
          operation: 'getReportAttempt',
          status: 200,
        }),
        expect.objectContaining({
          method: 'POST',
  ...
  it('marks status responses no-store so signed URLs cannot be cached', async () => {
    const service = {
      ...
      getReportAttempt: jest
        .fn()
        .mockResolvedValue({ id: 'attempt', status: 'ready', downloadUrl: 'https://private' }),
      retryReportDelivery: jest.fn(),
    };
    const controller = new ReportsController(service as never);
    await controller.execute({
      operation: 'getReportAttempt',
      request: { clerkPrincipal: principal } as never,
      query: {},
      body: {},
      params: { attemptId: '99000000-0000-4000-8000-000000000001' },
      response,
    });
