...
describeLiveDatabase('device registration repository', () => {
  ...
  const owner: ClerkPrincipal = {
    userId: 'device_registration_owner',
    sessionId: 'device_registration_session_a',
    factorAgeSeconds: 30,
  };
  const other: ClerkPrincipal = {
    userId: 'device_registration_other',
    sessionId: 'device_registration_session_b',
    factorAgeSeconds: 30,
  };
  const registration = {
  ...
  it('serializes an owner/fingerprint upsert race to one device', async () => {
    ...
    const rows = await asMigration((client) =>
      client.query<{ count: string }>(
        'select count(*) from public.user_devices where user_id = $1',
        [owner.userId],
      ),
    );
  ...
  it('rotates push tokens, protects stored values, and rejects cross-owner reuse', async () => {
    ...
    await expect(
      repository.registerDevice(other, {
        ...registration,
  ...
  it('reactivates only with a fresh linked session and pages deterministically', async () => {
    ...
    await expect(repository.registerDevice(owner, registration)).rejects.toThrow(
      'PUSH_TOKEN_CONFLICT',
    );
    ...
    const secondPage = await repository.listDevices(
      fresh,
      { lastSeenAt: next.lastSeenAt, id: next.id },
      1,
    );
