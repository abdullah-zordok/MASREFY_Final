...
describeLiveDatabase('device revocation', () => {
  ...
  const owner: ClerkPrincipal = {
    userId: 'device_revocation_owner',
    sessionId: 'device_revocation_session',
    factorAgeSeconds: 30,
  };
  const other: ClerkPrincipal = {
    userId: 'device_revocation_other',
    sessionId: 'device_revocation_other_session',
    factorAgeSeconds: 30,
  };
  const input = {
    deviceFingerprint: 'device-revocation-fingerprint',
    platform: 'ios' as const,
    appVersion: '1.0.0',
    pushToken: 'device-revocation-push-token',
    pushProvider: 'apns' as const,
  };
  ...
  it('requires recent auth before changing the current device', async () => {
    ...
    const stale = new IdentityService(
      repository,
      { revokeSession: jest.fn() } as never,
      {
  ...
  it('commits local denial before Clerk, survives outage, and retries naturally', async () => {
    ...
    const clerk = {
      revokeSession: jest
        .fn()
        .mockRejectedValueOnce(new Error('CLERK_PROVIDER_UNAVAILABLE'))
        .mockResolvedValue('revoked'),
    };
    const service = new IdentityService(
      repository,
      clerk as never,
      { get: jest.fn().mockReturnValue(600) } as never,
    );
    await expect(
      service.revokeDevice(owner, registered.device.id, 'device-revoke-outage'),
    ...
    await expect(
      service.revokeDevice(owner, registered.device.id, 'device-revoke-repeat'),
  ...
  it('returns a non-owner-safe not found result', async () => {
    const registered = await repository.registerDevice(owner, {
      ...input,
      deviceFingerprint: 'device-revocation-fingerprint-other-case',
      pushToken: undefined,
