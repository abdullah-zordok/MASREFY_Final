  ...
  return {
    ...
    listWork: jest
      ...
      .mockImplementation((kind: string) =>
        Promise.resolve(kind === 'report.email.deliver' && !delivered ? ['id'] : []),
      ),
    withWorkLock: jest
      .fn()
      .mockImplementation(
        async (_kind: string, _id: string, action: (work: never) => Promise<ReportWorkOutcome>) => {
          delivered = true;
...
describe('report email worker', () => {
  it('signs at send time and records one SMTP acceptance on replay', async () => {
    ...
    const smtp = {
      send: jest.fn().mockResolvedValue({
        providerMessageId: '<stable@example.test>',
    ...
    expect(outcome).toMatchObject({
      status: 'delivered',
      providerMessageId: '<stable@example.test>',
      event: { type: 'report.delivery_succeeded' },
    });
  });
  ...
  it.each([
    ...
    'classifies delivery failure without exposing provider content',
    async (failure, status, retryable) => {
      ...
      expect(outcome).toMatchObject({
        ...
        event: { type: 'report.delivery_failed', data: { retryable } },
      });
