...
describeLiveDatabase('report schedules', () => {
  ...
  it('creates idempotently, masks recipients, paginates, and isolates owners', async () => {
    ...
    expect(await repository.createSchedule(owner, command, 'schedule-key-1', 'request')).toEqual(
      first,
    );
  ...
  it('uses compare-and-set updates and rejects stale versions', async () => {
    ...
    await expect(
      repository.updateSchedule(owner, command, 'schedule-key-2', 'request'),
    ).resolves.toMatchObject({ version: 2, enabled: false });
    await expect(
      repository.updateSchedule(owner, { ...command, enabled: true }, 'schedule-key-3', 'request'),
    ).rejects.toMatchObject({ status: 409 });
  });

  it('deletes idempotently at the expected version', async () => {
    const page = await repository.listSchedules(owner, null, 25, 'list');
    const current = (page.items as Array<{ id: string; version: number }>)[0];
    const first = await repository.deleteSchedule(
      owner,
      current?.id ?? '',
      current?.version ?? 0,
      'schedule-key-delete',
    ...
    expect((await repository.listSchedules(owner, null, 25, 'list')).items).toHaveLength(0);
  });
});
