  ...
  it('keeps deletion active uniqueness and cooling-off in database constraints', async () => {
    await pool.withClient(async (client) => {
      ...
      try {
        ...
        await client.query(
          "insert into public.profiles(id,status) values('integration_delete_owner','active') on conflict do nothing",
        );
        await expect(
          client.query(`insert into private.account_deletion_requests(user_id,cooling_off_ends_at)
          values('integration_delete_owner',clock_timestamp()-interval '1 hour')`),
