  ...
  it('returns only owner-safe columns and rows in deterministic order', async () => {
    await pool.withClient(async (client) => {
      ...
      try {
        ...
        await client.query(
          "insert into public.profiles(id,status) values('security_event_owner','active'),('security_event_other','active') on conflict do nothing",
        );
        ...
        await client.query(
          'select set_config(\'request.jwt.claims\',\'{"role":"authenticated","sub":"security_event_owner","sid":"session"}\',true)',
        );
        await client.query('set local role authenticated');
        const page =
          await client.query(`select id,event_type,severity,metadata,occurred_at from public.security_events
        ...
        await expect(
          client.query('select ip_hash from public.security_events'),
