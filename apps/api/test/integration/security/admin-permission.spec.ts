...
describeLiveDatabase('Admin exact permission transaction', () => {
  ...
  it('allows only the exact canonical key', async () => {
    ...
    await expect(
      repository.assertAdminPermission(principal, 'audit.read'),
    ).resolves.toBeUndefined();
    await expect(
      repository.assertAdminPermission(principal, 'audit.logs.read'),
    ).rejects.toMatchObject({ code: '42501' });
    await expect(repository.assertAdminPermission(principal, 'audit.*')).rejects.toMatchObject({
      code: '42501',
    });
  });
  ...
  it('does not let a customer or an evaluator failure become an allow', async () => {
    await expect(
      repository.assertAdminPermission(
  ...
  it('removes permission immediately after assignment revoke', async () => {
    await pool.query(
      'update public.admin_role_assignments set revoked_at=clock_timestamp() where user_id=$1',
      [adminId],
    );
    await expect(
      repository.assertAdminPermission(
