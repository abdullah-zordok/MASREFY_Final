...
|---|---|---|
| Mobile auth/app shell | Mock or unavailable auth factory; persisted synthetic authenticated summaries accepted | Official Clerk live path only; legacy synthetic state rejected outside explicit demo/test |
...
| Admin shared client | Same-origin unauthenticated requests; development allows MSW; unhandled requests bypass | Clerk-authenticated configured API origin; production MSW build failure; unhandled mock error |
