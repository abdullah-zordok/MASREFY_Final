...
| Admin local performance | `npx playwright test --config=playwright.performance.config.ts` passed its unchanged 2,500 ms client-render and 200 ms interaction budgets in mock mode after isolating the browser server from inherited live credentials. Authenticated production-shell timing requires Clerk test credentials and remains open in `external-gates.md`. |
