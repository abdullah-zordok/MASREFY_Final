...
`(lease_until,session_id) where status='running'`, `(status,started_at)`.

### `private.raw_ingestion_payloads` (`I`)

| Column              | Type / default         | Rule                                                    |
| ------------------- | ---------------------- | ------------------------------------------------------- |
| `session_id`        | `uuid null`            | FK session cascade; null only for queued orphan cleanup |
...
| `scan_status`       | `text default 'clean'` | `clean`, `cleanup`, `purging`, `purged`                 |
| `expires_at`        | `timestamptz`          | 1..365 days from receipt policy                         |
| `purge_token`       | `uuid null`            | non-null only while a purge is leased                   |
| `purge_lease_until` | `timestamptz null`     | bounded retry lease for purge fencing                   |

Indexes: unique `(session_id,payload_hash)`, unique `storage_ref`, and partial
`(expires_at,purge_lease_until,id)` for registered retention, orphan cleanup, and
expired purge-lease reclaim.
