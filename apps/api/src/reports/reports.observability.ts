...
export function recordReportJob(job: string, outcome: string, durationMs?: number): void {
  ...
  if (durationMs !== undefined)
    recordPlatformMetric(REPORT_METRICS.duration, durationMs, { job, outcome });
}
...
export function recordReportBacklog(job: string, count: number): void {
  if (!jobs.has(job) || !Number.isSafeInteger(count) || count < 0)
    throw new Error('REPORT_METRIC_INVALID');
  recordPlatformMetric(REPORT_METRICS.backlog, count, { job });
}

export function safeReportLog(
  job: string,
  outcome: string,
  errorCode?: string,
  ...
  if (
    !jobs.has(job) ||
    !outcomes.has(outcome) ||
    (errorCode !== undefined && !/^[A-Z][A-Z0-9_]{1,79}$/.test(errorCode))
  )
