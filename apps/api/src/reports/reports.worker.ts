...
import { ReportsRepository, type ReportWorkClaim, type ReportWorkOutcome } from './reports.repository';
...
export class ReportsWorker implements OnModuleDestroy {
  ...
  start(): void {
    ...
    this.timer = setInterval(() => void this.runOnce().catch(() => undefined), this.config.getRequired('MASARIFI_REPORT_POLL_MS'));
    this.timer.unref();
  ...
  async onModuleDestroy(): Promise<void> { await this.stop(); }
  ...
  async runOnce(): Promise<void> {
    ...
    try {
      const now = new Date();
      const due = await this.repository.listDueSchedules(now, this.config.getRequired('MASARIFI_REPORT_BATCH_SIZE'));
      ...
      for (const schedule of due) {
        ...
        recordReportJob('report.schedule.enqueue', enqueued ? 'success' : 'locked', performance.now() - startedAt);
      }
      for (const kind of ['report.generate','report.email.deliver','report.output.expire'] as const) {
        const ids = await this.repository.listWork(kind, this.config.getRequired('MASARIFI_REPORT_BATCH_SIZE'));
        recordReportBacklog(kind, ids.length);
        for (const id of ids)
          await this.repository.withWorkLock(kind, id, (claim) =>
            kind === 'report.generate' ? this.generate(claim) : kind === 'report.email.deliver' ? this.deliver(claim) : this.expire(claim),
          );
      }
  ...
  private async generate(claim: ReportWorkClaim): Promise<ReportWorkOutcome> {
    ...
    try {
      ...
      const rendered = await renderReport(snapshot, snapshot.format, maximum, snapshot.format === 'pdf' ? this.font() : undefined);
      if (claim.status === 'generating') await this.storage.delete(this.storage.key(claim.id, claim.userId, snapshot.format));
      const stored = await this.storage.upload(claim.id, claim.userId, snapshot.format, rendered.contentType, rendered.body, maximum);
      ...
      return {
        status: 'ready', storageRef: stored.key,
        event: adminAggregate
          ? { type: 'export.ready', data: { attemptId: claim.id, format: snapshot.format, bytes: stored.bytes, expiresAt: claim.expiresAt, adminAggregate: true } }
          : { type: 'report.ready', data: { attemptId: claim.id, format: snapshot.format, bytes: stored.bytes, expiresAt: claim.expiresAt } },
      };
    } catch (error) {
      if (storedKey) try { await this.storage.delete(storedKey); } catch { /* expiry reconciliation will retry */ }
  ...
  private async expire(claim: ReportWorkClaim): Promise<ReportWorkOutcome> {
    try {
      ...
      return {
        status: 'expired',
        event: { type: 'report.expired', data: { attemptId: claim.id, expiredAt: new Date().toISOString() } },
      };
    } catch {
  ...
  private async deliver(claim: ReportWorkClaim): Promise<ReportWorkOutcome> {
    try {
      ...
      const snapshot = parseReportSnapshot(claim.snapshot, this.config.getRequired('MASARIFI_REPORT_MAX_BYTES'));
      if (snapshot.delivery !== 'email') throw new ReportsSmtpError('REPORT_EMAIL_INVALID');
      const recipient = claim.recipient ?? (await this.identity?.getIdentityUser(claim.userId))?.primaryEmail;
      if (!recipient) throw new ReportsSmtpError('REPORT_RECIPIENT_UNVERIFIED');
      const link = await this.storage.sign(claim.storageRef, this.config.getRequired('MASARIFI_REPORT_SIGNED_URL_SECONDS'));
      const accepted = await this.smtp.send(claim.id, recipient, link);
      recordReportJob('report.email.deliver', 'success');
      return {
        status: 'delivered', providerMessageId: accepted.providerMessageId,
        event: { type: 'report.delivery_succeeded', data: { attemptId: claim.id, acceptedByServerAt: accepted.acceptedByServerAt } },
      };
    } catch (error) {
      const failure = error instanceof ReportsSmtpError ? error : new ReportsSmtpError('REPORT_EMAIL_UNAVAILABLE', true);
      recordReportJob('report.email.deliver', failure.retryable ? 'retry' : 'failure');
      return {
        status: failure.retryable && claim.attemptCount < this.config.getRequired('MASARIFI_REPORT_MAX_ATTEMPTS') ? 'ready' : 'failed',
        errorCode: failure.code,
        event: { type: 'report.delivery_failed', data: { attemptId: claim.id, errorCode: failure.code, retryable: failure.retryable, attemptCount: claim.attemptCount } },
      };
    }
  }
