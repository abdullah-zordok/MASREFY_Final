...
const sensitiveKey =
  /authorization|cookie|credential|database.?url|password|secret|token|email|phone|session|subject|fingerprint|payload|cipher|otp|jwt/i;
const sensitiveValue =
  /(?:bearer\s+\S+|postgres(?:ql)?:\/\/[^\s:@]+:[^\s@]+@|[A-Z0-9._%+-]+@[A-Z0-9.-]+\.[A-Z]{2,}|\+[1-9][0-9]{7,14}|(?:sk|whsec)_(?:test|live)?_?[A-Za-z0-9_-]+)/gi;
