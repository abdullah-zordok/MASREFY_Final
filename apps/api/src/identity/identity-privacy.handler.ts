...
export class IdentityPrivacyHandler implements PrivacyDomainHandler {
  ...
  async *export(evidence: PrivacyEvidence): AsyncIterable<ExportEntry> {
    const snapshot = await this.workerTransaction(async (client) => {
      const [profile, preferences, onboarding, devices] = await Promise.all([
        client.query<Record<string, unknown>>(
          `select id,primary_email,phone_e164,display_name,locale,timezone,status,created_at,updated_at,version from public.profiles where id=$1`,
          [evidence.userId],
        ),
        client.query<Record<string, unknown>>(
          'select default_currency,language,theme,calendar,week_start,privacy_settings,version from public.user_preferences where user_id=$1',
          [evidence.userId],
        ),
        client.query<Record<string, unknown>>(
          'select step,completed_steps,completed_at,version from public.onboarding_progress where user_id=$1',
          [evidence.userId],
        ),
        client.query<Record<string, unknown>>(
          `select id,platform,app_version,device_name,trusted_at,last_seen_at,revoked_at,created_at,version
          from public.user_devices where user_id=$1 order by created_at,id limit 1000`,
          [evidence.userId],
        ),
      ]);
      return {
    ...
    yield {
      ...
      stream: jsonEntry(snapshot.profile),
    };
    yield {
      path: 'identity/preferences.json',
      mediaType: 'application/json',
      stream: jsonEntry(snapshot.preferences),
    };
    yield {
      path: 'identity/onboarding.json',
      mediaType: 'application/json',
      stream: jsonEntry(snapshot.onboarding),
    };
    yield {
      path: 'identity/devices.json',
      mediaType: 'application/json',
      stream: jsonEntry(snapshot.devices),
    };
  }
  ...
  async deleteAccount(evidence: PrivacyEvidence): Promise<DeletionOutcome> {
    const sessions = await this.workerTransaction(async (client) => {
      const rows = await client.query<{ clerk_session_id: string | null }>(
        'select clerk_session_id from public.user_devices where user_id=$1 and revoked_at is null',
        [evidence.userId],
      );
      return rows.rows.flatMap(({ clerk_session_id }) =>
    ...
    const anonymizedCount = await this.workerTransaction(async (client) => {
      await client.query(
        'update public.push_tokens set revoked_at=clock_timestamp() where user_id=$1 and revoked_at is null',
        [evidence.userId],
      );
      await client.query(
        `update public.user_devices set revoked_at=coalesce(revoked_at,clock_timestamp()),clerk_session_id=null
        where user_id=$1 and (revoked_at is null or clerk_session_id is not null)`,
        [evidence.userId],
      );
      const profile = await client.query(
        `update public.profiles set primary_email=null,phone_e164=null,display_name=null,status='deleted'
  ...
  async listRetentionCandidates(
    ...
    return this.workerTransaction(async (client) => {
      const rows = await client.query<{ id: string; updated_at: Date; version: string }>(
        `select id,updated_at,version from public.profiles
        where status='deleted' and updated_at<$1 and ($2::text is null or id>$2) order by id limit $3`,
  ...
  async applyRetention(
    ...
    const affected = await this.workerTransaction(async (client) => {
      const result = await client.query(
        `update public.profiles set primary_email=null,phone_e164=null,display_name=null
        where id=$1 and version=$2 and status='deleted' returning id`,
