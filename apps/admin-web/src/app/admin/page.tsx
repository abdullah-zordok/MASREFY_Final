...
} from "@/features/overview/report-exports";
...
const OVERVIEW_METRIC_COPY: Record<
  string,
  Record<Locale, { label: string; note?: string }>
> = {
  "unique-customers": {
...
const ACTIVITY_SUMMARIES: Record<string, Record<Locale, string>> = {
  "ACT-DEMO-001": {
    ar: "ترقية اشتراك تجريبي إلى Premium.",
    en: "Mock subscription upgraded to Premium.",
  },
  "ACT-DEMO-002": {
    ar: "تسجيل عميل تجريبي جديد.",
    en: "New mock customer registered.",
  },
  "ACT-DEMO-003": {
    ar: "فشل إشعار دفع تجريبي.",
    en: "Mock payment webhook failed.",
  },
  "ACT-DEMO-004": {
    ar: "تحديث قاعدة محلل تجريبي.",
    en: "Mock parser rule updated.",
  },
  "ACT-DEMO-005": { ar: "تغيير دور تجريبي.", en: "Mock role changed." },
  "ACT-DEMO-006": {
    ar: "اعتماد وصول دعم تجريبي.",
    en: "Mock support access approved.",
  },
  "ACT-DEMO-007": {
    ar: "اكتمال حذف حساب تجريبي.",
    en: "Mock account deletion completed.",
  },
  "ACT-DEMO-008": {
    ar: "تسجيل عميل تجريبي إضافي.",
    en: "Additional mock customer registered.",
  },
  "ACT-DEMO-009": {
    ar: "ترقية اشتراك تجريبية أخرى.",
    en: "Another mock subscription upgraded.",
  },
  "ACT-DEMO-010": {
    ar: "فشل إشعار دفع تجريبي إضافي.",
    en: "Additional mock payment webhook failed.",
  },
  "ACT-DEMO-011": {
    ar: "تحديث قاعدة محلل إضافي.",
    en: "Additional parser rule updated.",
  },
  "ACT-DEMO-012": {
    ar: "تسجيل عميل تجريبي ثالث.",
    en: "Third mock customer registered.",
  },
};

const ATTENTION_SUMMARIES: Record<string, Record<Locale, string>> = {
  "ATT-DEMO-queue-1": {
    ar: "تراكم تجريبي في قائمة انتظار التقارير.",
    en: "Mock backlog in the reporting queue.",
  },
  "ATT-DEMO-payment-1": {
    ar: "ارتفاع تجريبي في فشل المدفوعات.",
    en: "Mock increase in payment failures.",
  },
  "ATT-DEMO-incident-1": {
    ar: "حادث تجريبي حرج في مزود الذكاء الاصطناعي.",
    en: "Critical mock incident in the AI provider.",
  },
  "ATT-DEMO-import-1": {
    ar: "ارتفاع تجريبي في فشل الاستيراد.",
    en: "Mock increase in import failures.",
  },
  "ATT-DEMO-ai-1": {
    ar: "انقطاع تجريبي جزئي لمزود الذكاء الاصطناعي.",
    en: "Partial mock outage for the AI provider.",
  },
  "ATT-DEMO-security-1": {
    ar: "تنبيه أمان تجريبي يتطلب مراجعة.",
    en: "Mock security alert requires review.",
  },
  "ATT-DEMO-deletion-1": {
    ar: "فشل تجريبي في إكمال حذف حساب.",
    en: "Mock account deletion did not complete.",
  },
  "ATT-DEMO-support-1": {
    ar: "تذكرة دعم تجريبية ذات أولوية عالية.",
    en: "High-priority mock support ticket.",
  },
  "ATT-DEMO-info-1": {
    ar: "ملاحظة تشغيلية تجريبية.",
    en: "Mock operational note.",
  },
  "ATT-DEMO-admin-governance-1": {
    ar: "مراجعة حوكمة دور إداري تتطلب انتباها.",
    en: "Admin role governance review requires attention.",
  },
  "ATT-DEMO-settings-1": {
    ar: "إعدادات الصيانة مجدولة لمراجعة تجريبية.",
    en: "Maintenance settings are scheduled for mock review.",
  },
};
...
const CAPABILITY_LABELS: Record<string, Record<Locale, string>> = {
  ...
  "notification-listener": {
    ar: "مستمع الإشعارات",
    en: "Notification Listener",
  },
};

const CAPABILITY_CAVEATS: Record<string, Record<Locale, string>> = {
  shortcut: {
    ar: "يقيس اعتماد اختصارات iOS المجمعة فقط.",
    en: "Measures aggregate iOS shortcut adoption only.",
  },
  "share-extension": {
    ar: "اعتماد تجميعي لامتداد المشاركة.",
    en: "Aggregate adoption for the share extension.",
  },
  "sms-tracking": {
    ar: "تتبع تجميعي لرسائل Android.",
    en: "Aggregate tracking for Android messages.",
  },
  "notification-listener": {
    ar: "اعتماد تجميعي لمستمع إشعارات Android.",
    en: "Aggregate adoption for the Android notification listener.",
  },
};
...
function metricValue(metric: OverviewMetric, locale: Locale): string {
  if (metric.id === "mrr")
    return locale === "ar" ? "2.48 مليون SAR" : "2.48M SAR";
...
function versionLabel(version: string, locale: Locale): string {
  if (version === "غير معروف")
    return labelFor(VERSION_STATE_LABELS, locale, "unknown");
  return version;
}
...
export default function OverviewPage() {
  ...
  const [reportExport, setReportExport] = useState<
    AdminExportStatus | "pending" | null
  >(null);
  ...
  const importVolumeSeries = useMemo(
    () =>
      analytics.data?.comparisonTrends.find(
        (series) => series.id === "import-volume",
      ),
    [analytics.data],
  );
  const importVolumePoints = useMemo(
    () =>
      importVolumeSeries
        ? trendToChartPoints(importVolumeSeries).map((point) => ({
            ...point,
            secondary: point.previous,
          }))
        : [],
    [importVolumeSeries],
  );
  ...
  const attentionError =
    attention.error instanceof ApiError
      ? { code: attention.error.code }
      : undefined;
  const activityError =
  ...
  return (
    <div className="page overview-page" dir={direction}>
        ...
        actions={
          <>
            {platformOptions.data && (
              <PlatformFilter
                options={platformOptions.data.options}
                value={platform}
            ...
            {hasPermission(role, "data_requests.exports.manage") &&
              ...
              reportExport.downloadUrl ? (
                <a
                  className="button"
                  data-spec="overview-export-download"
                  href={reportExport.downloadUrl}
                  rel="noreferrer"
                >
                  <Download size={16} />
                  <span>
                ...
                <button
                  ...
                  onClick={async () => {
                    ...
                    try {
                      ...
                      setReportExport(
                        await reportExportsRepository.wait(accepted.attemptId),
                      );
                    } catch {
            ...
            <>
              <div className="metrics-grid">
                {metrics.slice(0, 4).map((metric) => (
                  <div
                    data-spec={
                      metric.id === "unique-customers"
                        ? "unique-customers-total"
                        : undefined
                    }
                    key={metric.id}
                  >
      ...
      <section
        ...
        aria-label={t("overview.customerAlertsAria")}
      >
        <div>
          <RegionState
            ...
            {analytics.data && (
              ...
              >
                <TrendChart
                  ...
                  compare={platform === "all"}
                />
              </ChartCard>
            )}
            {overlapNote && (
              <p
                className="region-warning"
                data-spec="overlap-warning"
                role="status"
              >
        ...
        <article className="card attention-card">
          <div className="card-heading">
            <div>
              ...
              <p>
                {t("overview.attentionOpenCases", {
                  count: attention.data?.totalItems ?? 0,
                })}
              </p>
            </div>
          </div>
                ...
                {attention.data.items.map((item) => {
                  ...
                  const body = (
                    <>
                      <div className="attention-title">
                        <h3>
                          {ATTENTION_SUMMARIES[item.id]?.[locale] ?? item.type}
                        </h3>
                  ...
                  return (
                    <div className="attention-item" key={item.id}>
                      {destination ? (
                        <Link href={destination}>{body}</Link>
                      ) : (
                        body
                      )}
                    </div>
      ...
      <section
        className="section-grid equal"
        aria-label={t("overview.importAnalyticsAria")}
      >
        <RegionState
          isPending={analytics.isPending}
          isError={analytics.isError}
                ...
                data={
                  subscription
                    ? subscription.distribution.map((d, index) => ({
                        name: subscriptionPlanLabel(d.plan, index, locale),
              ...
              <div className="legend">
                {subscription?.distribution.map((d, index) => (
                  <span key={d.plan}>
                    <i
                      style={{
                        background: `var(--chart-series-${(index % 4) + 1})`,
                      }}
                    />
                    {subscriptionPlanLabel(d.plan, index, locale)}{" "}
            ...
            >
              <DonutChart
                data={
                  customers
                    ? [
                        {
                          name: t("overview.iosOnly"),
                          value: customers.iosOnlyCustomers,
                        },
                        {
                          name: t("overview.androidOnly"),
                          value: customers.androidOnlyCustomers,
                        },
                        {
                          name: t("overview.multiPlatform"),
                          value: customers.multiPlatformCustomers,
                        },
                      ]
                    : []
                }
              ...
              <div className="legend">
                <span>
                  ...
                  {t("overview.iosOnly")} {customers?.iosOnlyCustomers ?? 0}
                </span>
                <span>
                  <i style={{ background: "var(--chart-series-2)" }} />
                  {t("overview.androidOnly")}{" "}
                  {customers?.androidOnlyCustomers ?? 0}
                </span>
                <span>
                  <i style={{ background: "var(--chart-series-3)" }} />
                  {t("overview.multiPlatform")}{" "}
                  {customers?.multiPlatformCustomers ?? 0}
                </span>
              </div>
            </ChartCard>
          </RegionState>
      ...
      <section
        className="section-grid equal"
        data-spec="adoption-summary"
        aria-label={t("overview.adoptionAria")}
      >
        <RegionState
          isPending={analytics.isPending}
          isError={analytics.isError}
          region={
            findRegion(analytics.data?.regions, "adoption") ?? customersRegion
          }
          error={analyticsError}
          onRetry={() => analytics.refetch()}
          ...
          <ChartCard
            ...
            summary={
              analytics.data?.versions.length
                ? analytics.data.versions
                    .map(
                      (item) =>
                        `${versionLabel(item.version, locale)} ${percent(item.share)}`,
                    )
                    .join(listSeparator)
            ...
            <div className="legend">
              {(analytics.data?.versions ?? []).map((item, index) => (
                <span key={`${item.platform}-${item.version}`}>
                  ...
                  />
                  {versionLabel(item.version, locale)} ·{" "}
                  {labelFor(VERSION_STATE_LABELS, locale, item.supportState)} ·{" "}
                  {percent(item.share)}
                </span>
              ))}
            </div>
            ...
            <div className="health-list">
              {(analytics.data?.capabilities ?? []).map((item) => (
                <div
                  ...
                  key={`${item.platform}-${item.capability}`}
                >
                  <div className="health-name">
                    {labelFor(CAPABILITY_LABELS, locale, item.capability)}
                  </div>
                  <div className="health-cell">
                    <small>{t("overview.enabled")}</small>
                    <strong className="numbers ltr">
                      {item.enabledPopulation.toLocaleString("en-US")}
                    </strong>
                  </div>
                  <div className="health-cell">
                    <small>{t("overview.eligible")}</small>
                    <strong className="numbers ltr">
                      {item.eligiblePopulation.toLocaleString("en-US")}
                    </strong>
                  </div>
                  <div className="health-cell">
                    <small>{t("overview.rate")}</small>
                    <strong className="numbers ltr">
                      {percent(item.rate)}
                    </strong>
                  </div>
                  <small>
                    {labelFor(CAPABILITY_CAVEATS, locale, item.capability)}
                  </small>
                </div>
              ))}
              {(analytics.data?.devices ?? []).map((item) => (
                ...
                >
                  <div className="health-name">
                    {labelFor(DEVICE_CATEGORY_LABELS, locale, item.category)}
                  </div>
                  <div className="health-cell">
                    <small>{t("overview.devices")}</small>
                    <strong className="numbers ltr">
                      {item.deviceCount.toLocaleString("en-US")}
                    </strong>
                  </div>
                  <div className="health-cell">
                    <small>{t("overview.share")}</small>
                    <strong className="numbers ltr">
                      {percent(item.share)}
                    </strong>
                  </div>
              ...
              {analytics.data &&
                analytics.data.capabilities.length === 0 &&
                analytics.data.devices.length === 0 && (
                  <p className="region-warning">
                    {t("overview.capabilitiesPrompt")}
                  </p>
                )}
            </div>
          </article>
      ...
      <section
        className="section-grid"
        aria-label={t("overview.operationalAria")}
      >
        <article className="card">
          <div className="card-heading">
            <div>
              ...
              <p>
                <span data-spec="global-health-label">
                  {t("overview.globalLabel")}
                </span>{" "}
                · {t("overview.globalMeasurement")} ·{" "}
                {t("overview.platformUnaffected")}
              </p>
            </div>
            ...
            <div className="health-list">
              {summary.data?.serviceHealth.map((service) => (
                <div className="health-row" key={service.service}>
                  <div className="health-name">
                    <span
                      className={`health-dot ${service.status === "operational" ? "" : service.status === "degraded" || service.status === "maintenance" ? "warn" : "danger"}`}
                    />
                    {serviceLabel(service.service, locale)}
                  </div>
                  <div className="health-cell">
                    <small>{t("overview.status")}</small>
                    <StatusBadge
                      status={
                        STATUS_BY_SERVICE[service.status] ?? "operational"
                      }
                    />
                  </div>
                  <div className="health-cell">
                  ...
                  <div className="health-cell">
                    <small>{t("overview.response")}</small>
                    <strong className="numbers ltr">
                      {service.latencyMs} ms
                    </strong>
                  </div>
                  <div className="health-cell">
                    <small>{t("overview.error")}</small>
                    <strong className="numbers ltr">
          ...
          >
            {activity.data && activity.data.items.length > 0 && (
              <>
                        ...
                        <div>
                          {destination ? (
                            <Link href={destination}>
                              <strong>
                                {ACTIVITY_SUMMARIES[item.id]?.[locale] ??
                                  item.eventType}
                              </strong>
                            </Link>
                          ) : (
                ...
                >
                  <Clock3 size={14} />{" "}
                  {t("overview.activityFooter", {
                    period: getPeriodLabel(locale, period),
                  })}{" "}
                  ·{" "}
                  <span data-spec="activity-page">
