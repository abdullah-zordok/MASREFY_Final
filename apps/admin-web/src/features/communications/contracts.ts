...
export const sharedIdSchema = z
  .string()
  .regex(/^[A-Z]{2,4}-\d{4,6}-[A-Z0-9]+$/);
...
export const createKiBLimitSchema = (maxKiB: number) =>
  z.string().refine((val) => new Blob([val]).size <= maxKiB * 1024, {
...
export const attachmentSchema = z
  .object({
    ...
    declaredSizeBytes: z
      ...
      .min(0)
      .max(10 * 1024 * 1024),
    status: z.enum(["pending", "clean", "rejected", "failed"]).optional(),
  })
  .strict();
...
export const engagementAdminActionSchema = z
  .object({
    action: z.enum([
      "test",
      "assign",
...
export const abuseSeveritySchema = z.enum([
  "low",
  "medium",
  "high",
  "critical",
...
export const apiErrorSchema = safeErrorSchema
  .extend({
...
export const communicationRegionSchema = z
  .object({
    availability: z.enum([
      "available",
      "empty",
...
export const communicationMetricSchema = z
  .object({
    key: z.string().max(60),
...
export const communicationRecordSchema = z
  .object({
    ...
    locale: z.enum(["ar", "en", "both", "unknown"]).default("unknown"),
...
export const communicationPageSchema = z
  .object({
...
export const communicationOverviewSchema = z
  .object({
...
export const communicationDetailSchema = communicationRecordSchema
  .extend({
    body: safeTextSchema.pipe(z.string().max(8192)),
    notes: z
      .array(safeTextSchema.pipe(z.string().max(8192)))
      .max(100)
      .default([]),
    attachments: z.array(attachmentSchema).max(600).default([]),
    messages: z
      .array(
        z
          .object({
...
export const audiencePreviewRequestSchema = z
  .object({
