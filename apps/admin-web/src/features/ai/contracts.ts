...
export const reasonTextSchema = boundedUtf8(500).refine(
  (text) => text.length >= 3,
  {
...
export const aiOverviewQuerySchema = z
  .object({
...
export const apiErrorSchema = z
  .object({
    ...
    fieldErrors: z
      .record(z.string(), z.array(z.string().max(160)).max(10))
      .optional(),
...
export const moneyEstimateSchema = z
  .object({
  ...
  .superRefine((money, context) => {
    ...
    if (normalizedFields !== 0 && normalizedFields !== 3) {
      context.addIssue({
        code: "custom",
        message: "normalized money fields must appear together",
      });
    }
  });
...
export const aiOverviewSchema = z
  .object({
...
export const fallbackRouteSchema = z
  .object({
...
export const fallbackRoutesSchema = z
  .array(fallbackRouteSchema)
  .min(1)
  .max(180)
  .superRefine((routes, context) => {
    ...
    for (const [index, route] of routes.entries()) {
      ...
      if (priorities.has(priorityKey)) {
        context.addIssue({
          code: "custom",
          message: "fallback priorities must be unique per feature/locale",
          path: [index, "priority"],
        });
      }
      ...
      if (!route.compatible) {
        context.addIssue({
          code: "custom",
          message: "fallback route must be compatible",
          path: [index, "compatible"],
        });
      }
    ...
    for (const route of routes) {
      if (!terminalCoverage.has(`${route.feature}:${route.locale}`)) {
        context.addIssue({
          code: "custom",
          message: "fallback chain requires terminal coverage",
        });
        break;
      }
    }
...
const fullProviderSchema = providerCoreSchema
  .extend({
    accessLevel: z.literal("full"),
    health: z.enum(["healthy", "degraded", "partial_outage", "unavailable"]),
    defaultModelId: safeAiIdSchema
      .refine((id) => id.startsWith("AIM-"))
      .optional(),
    features: z.array(aiFeatureSchema).min(1).max(20).optional(),
    locales: z.array(localeScopeSchema).min(1).max(2).optional(),
    ...
    actions: z
      .array(z.enum(["activate", "deactivate", "update_fallback"]))
      .max(3),
  })
  .strict();

const contextProviderSchema = providerCoreSchema
  .extend({
...
const aggregateProviderSchema = providerCoreSchema
  .extend({
...
const fullModelSchema = modelCoreSchema
  .extend({
    accessLevel: z.literal("full"),
    features: z.array(aiFeatureSchema).min(1).max(20).optional(),
    locales: z.array(localeScopeSchema).min(1).max(2).optional(),
    ...
    eligible: z.boolean(),
    actions: z
      .array(z.enum(["assign", "unassign", "activate", "deactivate"]))
      .max(4),
  })
  .strict();

const aggregateModelSchema = modelCoreSchema
  .extend({
    accessLevel: z.literal("aggregate"),
    inputCost: moneyEstimateSchema,
    outputCost: moneyEstimateSchema,
...
export const aiActionContextSchema = z
  .object({
...
export const aiAdminResourceSchema = z
  .object({
    id: z.uuid(),
...
export const aiAdminPageSchema = z
  .object({
...
export const aiActionResultSchema = z
  .object({
...
export const aiSafetyDefinitionSchema = z
  .object({
    conditions: z
      .array(
        z
  ...
  .refine(
    (definition) =>
      new TextEncoder().encode(JSON.stringify(definition)).length <= 8192,
    { message: "safety definition exceeds 8 KiB" },
  );

const aiOperationalRecordBaseSchema = z
  .object({
    id: safeAiIdSchema,
...
export const aiOperationalRecordSchema =
  aiOperationalRecordBaseSchema.superRefine((record, context) => {
    if (
      ...
      context.addIssue({
        code: "custom",
        message:
          "aggregate projections cannot contain record-level sensitive fields",
      });
    }
  });
...
export const aiPromptDetailSchema = aiOperationalRecordBaseSchema
  .extend({
    resource: z.literal("prompts"),
    sanitizedPreview: boundedUtf8(4096),
    variables: z
      .array(z.string().regex(/^[A-Za-z][A-Za-z0-9_]{0,39}$/))
      .max(30),
...
export const aiOperationalActionRequestSchema = z
  .object({
    context: aiActionContextSchema,
    action: z.enum([
      "activate",
...
export type AiOperationalActionRequest = z.infer<
  typeof aiOperationalActionRequestSchema
>;
