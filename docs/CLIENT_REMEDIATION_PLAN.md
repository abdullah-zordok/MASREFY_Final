...
|  30 | Category deletion without confirmation: **Implemented locally on 2026-09-05.** Archive/merge fetch an owner-scoped authoritative linked-transaction count before confirmation; execution rechecks count/version, and merge atomically reclassifies all owned transaction headers while preserving postings/revisions/audit/outbox. | M/B/DB/API/UX. pgTAP, live repository integration, API contract/security, live/mock adapter, localized UI tests. BE-004/005 and Mobile 004/014 own the evidence. | Complete for the approved archive/merge + usage-count decision. System/foreign/missing/stale inputs fail safely. Separate undo and bulk actions remain deliberately out of scope. |
