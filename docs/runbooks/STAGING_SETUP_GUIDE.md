...
| **PASS** | All canonical migrations are applied to hosted staging. | Supabase migration history | Keep hosted history aligned with the timestamped files; never use `db reset --linked`. | Remote history has exactly 71 entries matching `supabase/migrations` by version and name, with no missing or extra migration. Latest: `20260924071023_configure_free_text_ai`. |
...
Hosted structural evidence captured on 2026-09-22 and refreshed on 2026-09-24:
...
| Priority | What is missing / why | Where | Safe action | Verification evidence |
| --- | --- | --- | --- | --- |
| **PASS (key and guardrail configured; runtime canary pending)** | OpenRouter workspace `Masarifi Staging` has one staging worker key with a USD 2 total cap and 2026-10-24 expiry. Guardrail `Masarifi Free ZDR` is assigned to that key and permits only `Qwen3.8 27B (free)` through ModelRun, requires ZDR, and rejects free endpoints that train on requests. The pre-existing general key remains untouched. | OpenRouter workspace `masarifi-staging`, guardrail detail, and future `/etc/masarifi/worker.env` | Store the key only in the restricted worker environment when the staging host exists. Keep provider execution disabled until the redacted canary and governed prompt publication pass. | Guardrail detail showed one assigned API key, one eligible provider, one eligible model, ZDR enabled for non-frontier models, free-training disabled, and no paid model in the allowlist. No secret value was recorded. |
| **PASS (catalog and database configuration)** | Current model capabilities, ZDR eligibility, provider policy, and prices were revalidated. `qwen/qwen3.8-27b:free` supports structured output and is served by ModelRun, which OpenRouter lists as no-training and zero-retention. | OpenRouter model/provider catalog, hosted Supabase, and Admin AI configuration | Use the free model for staging text workloads only. Keep Voice disabled until a compliant audio model and redacted live canary are approved. Revalidate metadata immediately before enablement. | Migration `20260924071023` configured all five disabled text routes with the free model, no fallback, a ModelRun-only allowlist, and zero prompt/completion price ceilings. Hosted verification returned five matching routes, zero enabled routes, and zero security-advisor lints. Voice remains on its separate disabled Google route. |
...
Current disabled Voice configuration is primary `google/gemini-2.5-flash` with `google/gemini-2.5-flash-lite` fallback and a Google-only allowlist; its 128k input/1,200 output-token bounds, 120-second route timeout, and USD 25 monthly route budget are unchanged. All five text workloads now use `qwen/qwen3.8-27b:free` with no fallback, a ModelRun-only allowlist, and zero prompt/completion price ceilings for staging. Existing paid model records remain available for a separately approved Production configuration later; they are not reachable from the staging key or routes. The repository's default accepted-work quota is five AI items per user in a rolling 24 hours, its global monthly guard is USD 200, and budget events are expected at 70/85/95 percent.
